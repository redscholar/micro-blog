...
func InitService() micro.Service {
	initCmd()
	initConfig()
	etcdCert, _ := tls.LoadX509KeyPair(util.ComParam.EtcdCertFile, util.ComParam.EtcdKeyFile)
	transportCert, _ := tls.LoadX509KeyPair(util.ComParam.TransportCertFile, util.ComParam.TransportKeyFile)
	brokerCert, _ := tls.LoadX509KeyPair(util.ComParam.BrokerCertFile, util.ComParam.BrokerKeyFile)
	auth := jwt.NewAuth(
		auth.Addrs(),
		auth.Namespace("blog"),
	...
	svc := micro.NewService(
			...
			server.NewServer(
				...
				server.WrapSubscriber(func(subscriberFunc server.SubscriberFunc) server.SubscriberFunc {
					return func(ctx context.Context, msg server.Message) error {
						newCtx, s := trace.DefaultTracer.Start(ctx, service)
						s.Type = trace.SpanTypeRequestInbound
						defer trace.DefaultTracer.Finish(s)
		...
		micro.Registry(
			registryEtcd.NewRegistry( // 设置etcd注册中心
				registry.Addrs(util.ComParam.EtcdAddr...), // etcd 地址。默认127.0.0.1:2379
		...
		micro.Cache(
			cacheRedis.NewCache(
				...
				//cache.Items(nil),
				cache.WithAddress(util.ComParam.RedisAddr[0]),
		...
		micro.Profile(http.NewProfile(profile.Name(service))),
...
func initCmd() {
	cmd.DefaultCmd.App().Action = util.InitComParam
	cmd.DefaultCmd.App().Flags = append(cmd.DefaultCmd.App().Flags, util.ComFlags...)
	cmd.DefaultCmd.Init()
}
